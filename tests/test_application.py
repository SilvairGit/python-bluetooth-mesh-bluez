...
from bluetooth_mesh.network.crypto import ApplicationKey, NetworkKey
...
@pytest.fixture(name="dbus")
def fixture_dbus(monkeypatch, node, application):
    ...
    class MockNetworkInterface(MockInterface):
        ...
        async def call_attach(self, _app_root, token):
        ...
        async def call_import(
            self, _app_root, uuid, _dev_key, _net_key, _net_index, _flags, _iv_index, unicast
        ):
...
def fixture_application(event_loop):
    class MockElement(Element):
        LOCATION = 0

    class MockApplication(Application):  # pylint: disable=too-many-ancestors
        ...
        def subnet_keys(self):
            return [self.primary_net_key]

        @property
        def app_keys(self):
            return [(0, 0, ApplicationKey(bytes.fromhex("3216d1509884b533248541792b877f98")))]
        ...
        def display_numeric(self, kind, number):
            pass

        def prompt_static(self, kind):
            raise NotImplementedError

        def prompt_numeric(self, kind):
            raise NotImplementedError
...
async def test_attach_bad_token(application, _dbus, node):
    async with application:
        with pytest.raises(dbus_next.errors.DBusError, match="Invalid token"):
            await application.attach(node.token + 1)


async def test_attach_good_token(application, _dbus, node):
    async with application:
        await application.attach(node.token)
        assert (
            application.node_interface._interface.proxy_object.object_path  # pylint: disable=protected-access
            == node.path
        )
...
class TestImportOnConnect:
    async def test_import(self, application, _dbus):
        async with application:
    ...
    async def test_missing_address(self, application, _dbus):
...
class TestAttachOnConnect:
    async def test_read_address_after_attach(self, application, _dbus, node):
        application.address = None
    ...
    async def test_import_new_on_attach_with_bad_token(self, application, _dbus, node):
    ...
    async def test_import_existing_with_bad_token(self, application, _dbus, node):
        application.token_ring.token = node.token + 1
