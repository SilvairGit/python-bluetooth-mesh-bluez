...
from bluetooth_mesh.bluez.interfaces import (
    AclInterface,
...
class DBusMixin:
    ...
    async def dbus_connect(self) -> None:
        ...
        dbus_service = self.bus.get_proxy_object(DBusService.NAME, DBusService.PATH, introspection)
...
class Application(
    ...
    async def _get_acl_interface(self):
        ...
        tcp_server = [node.name for node in mesh_introspection.nodes if node.name.startswith("tcpserver_")]
        if not tcp_server:
    ...
    async def dbus_connected(self, owner):
        introspection = await self.bus.introspect(self.DBUS_SERVICE.NAME, self.DBUS_SERVICE.PATH)
        service = self.bus.get_proxy_object(self.DBUS_SERVICE.NAME, self.DBUS_SERVICE.PATH, introspection)
    ...
    async def connect(
        ...
        **kwargs,
        ...
        await self.management_interface.import_remote_node(self.address, len(self.ELEMENTS), self.dev_key)
    ...
    async def attach(self, token: int, *, socket_pair=False, socket_path: Optional[str] = None):
        ...
        if socket_pair:
            path, configuration, sock = await self.network_interface.attach_fd("/", token)
            self._add_reader(sock)
        elif socket_path:
            path, configuration, sock = await self.network_interface.attach_unix("/", token, socket_path)
            self._add_reader(sock)
        else:
        ...
        for element, models_configs in configuration.items():
            for model_id, model_config in models_configs.items():
                self.elements[element].update_model_configuration(model_id, model_config)
        ...
        def _read_message() -> Any:
            while True:
                ...
                if dev_key:
                    ...
                    self.elements[element].dev_key_message_received(source, remote, net_index, data)
                else:
                    self.elements[element].message_received(source, app_index, destination, data)
