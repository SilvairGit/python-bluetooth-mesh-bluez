...
from bluetooth_mesh.bluez.models.base import Model
from bluetooth_mesh.bluez.utils import ModelOperationError, ProgressCallback
...
class ConfigClient(Model):
    ...
    async def add_app_key(
        ...
        return AppKeyStatus(status[params_name]["net_key_index"], status[params_name]["app_key_index"])
...
class DebugClient(Model):
    ...
    async def get_arap_content(
        ...
        *,
        ...
        return {
            node: None if isinstance(result, Exception) else result[status_opcode.name.lower()]["data"]
            for node, result in results.items()
        }
...
class GenericOnOffClient(Model):
    ...
    async def set_onoff(
        ...
        status = await self.query(request, status, send_interval=send_interval, timeout=1)
    ...
    async def get_light_status(
        ...
        *,
        ...
        return {
            node: None if isinstance(result, Exception) else result[status_opcode.name.lower()]
            for node, result in results.items()
        }
...
class LightExtendedControllerSetupClient(Model):
    ...
    async def get_property(
        ...
        *,
        ...
        return {
            node: None if isinstance(result, Exception) else result[params_name]["payload"]["value"]
            for node, result in results.items()
        }
...
class TimeClient(Model):
    ...
    async def set_time_role(self, nodes: Sequence[int], app_index: int, time_role: TimeRole):
        requests = {
            node: partial(
                self.send_app,
                node,
