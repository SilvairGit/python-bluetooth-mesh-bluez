...
class Model:
    ...
    def dev_key_message_received(self, source: int, remote: bool, net_index: int, message: ParsedMeshMessage):
        self.logger.debug(
    ...
    def expect_dev(
        ...
        def dev_message_received(_source: int, _net_index: int, message: ParsedMeshMessage):
    ...
    async def send_app(self, destination: int, app_index: int, opcode: int, params: MessageDescription):
        """
    ...
    async def _send_dev(self, destination: int, remote: bool, net_index: int, data: bytes):
        self.logger.debug(
        ...
        await self._node_interface.dev_key_send(self.element.path, destination, remote, net_index, data)

    async def send_dev(self, destination: int, net_index: int, opcode: int, params: MessageDescription):
        """
    ...
    async def subscribe(
        ...
        if subscription_address in self.configuration.subscriptions:
            ...
            return ModelSubscriptionStatus(element_address, subscription_address, type(self))

        return await self.element.application.subscribe_model(subscription_address, model=self)
