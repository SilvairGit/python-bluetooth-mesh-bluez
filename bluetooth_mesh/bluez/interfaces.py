...
def extract_model_config(configuration):
    ...
    for key, variant in configuration.items():
        ...
        if key == "PublicationPeriod":
            model_conf_dict["publication_period"] = timedelta(milliseconds=variant.value)
            continue
...
class ElementInterface(ServiceInterface):
    ...
    def message_received(self, source: "q", key_index: "q", destination: "v", data: "ay"):
    ...
    def dev_key_message_received(self, source: "q", remote: "b", net_index: "q", data: "ay"):
...
class NetworkInterface:
    ...
    async def attach_unix(
        ...
        path, configuration = await self._interface.call_attach_unix(app_defined_root, token, socket_path)
...
class NodeInterface:
    ...
    async def add_net_key(
        ...
        await self._interface.call_add_net_key(element_path, destination, subnet_index, net_index, update)
    ...
    async def add_app_key(
        ...
        await self._interface.call_add_app_key(element_path, destination, app_index, net_index, update)
...
class ManagementInterface:
    ...
    async def import_app_key(self, net_index: int, app_index: int, app_key: ApplicationKey) -> None:
    ...
    async def import_remote_node(self, primary: int, count: int, device_key: DeviceKey) -> None:
...
class AccessControlListInterface:
    def __init__(self, tcpserver_service):
        self._interface = tcpserver_service.get_interface("org.bluez.mesh.AccessControlList1")

    async def grant_access(self, uuid: UUID, dev_key: DeviceKey, net_key: NetworkKey) -> int:
        return await self._interface.call_grant_access(uuid.bytes, dev_key.bytes, net_key.bytes)
...
class ConnectionStatInterface:
    def __init__(self, tcpconnection_service):
        self._interface = tcpconnection_service.get_interface("org.bluez.mesh.ConnectionStat1")
